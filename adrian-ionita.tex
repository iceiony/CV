...
\noindent\emph{Data science engineer focused on computational modeling with a strong background in software engineering.
...
\ind \hspace{0.354in} \footnotesize  \textbf{R}, Python, \textbf{C/C++}, \textbf{SQL}, MATLAB, AWS, Docker, JavaScript, C\#, CUDA, ElasticSearch, MongoDB \normalsize 
...
\begin{itemize*}
    \item Deriving from raw trade data the exact logic needed for composing candlesticks, giving users access to a historic view of the trading market. 
    \item Developing an algorithm for market market liquidity by categorising trades, enabling traders to have a more representative view of liquidity.
    \item Investigating anomaly detection on trade data, helping the company satisfy compliance and regulatory obligations.
    \item Time series predictions of price correlations used in calculating the Value at Risk (VaR), thus estimating next day's possible Profit and Loss (P\&L).
\end{itemize*}
\ind \hspace{0.35in} \footnotesize \emph{Key Technologies: Python, Jupyter Lab, Pandas, Conda, R}

\normalsize
\medskip
\ind R Developer, LLOYDS (via EPAM) - \emph{contract} \hfill Mar 2020 - Oct 2021
\begin{itemize*}
    \item Supporting the capital modelling team in a smooth migration to Azure.
    \item Optimising R functionality to bring large performance improvements in Azure. 
    \item Training acturials on R best practices, improving the reliability of their code and day to day operations.
\end{itemize*}
\ind \hspace{0.35in} \footnotesize \emph{Key Technologies: R, Rcpp, C++, SQL, RSConnect, Tidyverse, Testing Automation}
...
\normalsize
\medskip
\ind R Developer / Data Scientist, Deutsche Bank (via HCL) - \emph{contract} \hfill July 2018 - Apr 2019
\begin{itemize*}
...
\end{itemize*}
\ind \hspace{0.35in} \footnotesize \emph{Key Technologies: C++, R, Dynet, ctree, algorithms }
